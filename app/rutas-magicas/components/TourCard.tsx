...
export default function TourCard({ tour, puebloSlug, className = '' }: TourCardProps) {
  return (
    <motion.div 
      ...
      className={`group bg-white dark:bg-gray-800 rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col h-full overflow-hidden border border-gray-100 dark:border-gray-700 hover:border-cyan-200 dark:hover:border-cyan-900/50 ${className}`}
    >
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4">
          <h3 className="text-xl font-bold text-white drop-shadow-md">{tour.title}</h3>
          <div className="flex items-center mt-2 space-x-2">
            <span className={`text-xs px-2 py-1 rounded-full font-medium ${difficultyColors[tour.difficulty].bg} ${difficultyColors[tour.difficulty].text}`}>
              {tour.difficulty}
            </span>
            {tour.rating && (
        ...
        <div className="mt-auto space-y-3">
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div className="flex items-center text-gray-600 dark:text-gray-300">
              ...
              <span className="truncate">Hasta {tour.capacity.max} personas</span>
            </div>
            
            <div className="flex items-center text-gray-600 dark:text-gray-300">
              <MapPin className="w-4 h-4 mr-2 text-cyan-500 flex-shrink-0" />
              <span className="truncate" title={tour.meetingPoint}>{tour.meetingPoint}</span>
            </div>
                ...
                <div>
                  ...
                  {tour.price.child && (
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      Niños: ${tour.price.child.toLocaleString()}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  <Link 
                    ...
                    aria-label={`Reservar ahora el tour ${tour.title}`}
                  >
